class ActionResolver::InvestigateSelfGeneratedActionTypeParams < ActionResolver
  ...
  def resolve(valid_results_hash, void_results_hash, city, trigger_id)
    # investigate vs. self generated action type params

    logger.info('MANUAL LOG - ' + self.class.name + '#' + __method__.to_s())

    self_generated_action_type_params_per_resident_id = {}

    unless valid_results_hash[ActionResultType::Investigate].nil?
    ...
    unless void_results_hash[ActionResultType::Investigate].nil?
  ...
  def decrease_available_actions(results_hashes, self_generated_action_type_params_per_resident_id)
    results_hashes[ActionResultType::Investigate].each { |result_hash|
      ...
      action_type_id_string = ActionType::INVESTIGATE.to_s()

      investigate_action_type_params_hash = self_generated_action_type_params_per_resident_id[resident_id][:result][ActionResultType::SelfGenerated::ActionTypeParams::KEY_ACTION_TYPES_PARAMS][role_id_string][action_type_id_string]
      if investigate_action_type_params_hash[ActionType::Investigate::PARAM_LIFETIME_ACTIONS_COUNT]>0
        investigate_action_type_params_hash[ActionType::Investigate::PARAM_LIFETIME_ACTIONS_COUNT] -= 1
      else
  ...
  def modify_action_type_params(action)
    if action.action_type_params.action_type_params_hash[ActionType::Investigate::PARAM_LIFETIME_ACTIONS_COUNT] > 0
      action.action_type_params.action_type_params_hash[ActionType::Investigate::PARAM_LIFETIME_ACTIONS_COUNT] -= 1
