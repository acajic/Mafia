class ActionResolver::ProtectSelfGeneratedActionTypeParams < ActionResolver
  ...
  def decrease_available_actions(results_hashes, self_generated_action_type_params_per_resident_id)
    results_hashes[ActionResultType::Protect].each { |result_hash|
      ...
      action_type_id_string = ActionType::PROTECT.to_s()

      protect_action_type_params_hash = self_generated_action_type_params_per_resident_id[resident_id][:result][ActionResultType::SelfGenerated::ActionTypeParams::KEY_ACTION_TYPES_PARAMS][role_id_string][action_type_id_string]
      if protect_action_type_params_hash[ActionType::Protect::PARAM_LIFETIME_ACTIONS_COUNT]>0
        protect_action_type_params_hash[ActionType::Protect::PARAM_LIFETIME_ACTIONS_COUNT] -= 1
      else
  ...
  def modify_action_type_params(action)
    if action.action_type_params.action_type_params_hash[ActionType::Protect::PARAM_LIFETIME_ACTIONS_COUNT] > 0
      action.action_type_params.action_type_params_hash[ActionType::Protect::PARAM_LIFETIME_ACTIONS_COUNT] -= 1
