class ActionResolver::SheriffIdentitiesSelfGeneratedActionTypeParams < ActionResolver
  ...
  def resolve(valid_results_hash, void_results_hash, city, trigger_id)
    # sheriff identities vs. self generated action type params

    logger.info('MANUAL LOG - ' + self.class.name + '#' + __method__.to_s())

    self_generated_action_type_params_per_resident_id = {}

    unless valid_results_hash[ActionResultType::SheriffIdentities].nil?
    ...
    unless void_results_hash[ActionResultType::SheriffIdentities].nil?
      ...
      void_results_hash[ActionResultType::SheriffIdentities] = void_results_hash[ActionResultType::SheriffIdentities].select{ |result_hash|
        result_hash[:result][ActionResultType::SheriffIdentities::KEY_SUCCESS]
      }
    end
  ...
  def decrease_available_actions(results_hashes, self_generated_action_type_params_per_resident_id)
    results_hashes[ActionResultType::SheriffIdentities].each { |result_hash|
      ...
      action_type_id_string = ActionType::SHERIFF_IDENTITIES.to_s()
      ...
      if sheriff_identities_action_type_params_hash[ActionType::SheriffIdentities::PARAM_LIFETIME_ACTIONS_COUNT]>0
        sheriff_identities_action_type_params_hash[ActionType::SheriffIdentities::PARAM_LIFETIME_ACTIONS_COUNT] -= 1
      else
  ...
  def modify_action_type_params(action)
    if action.action_type_params.action_type_params_hash[ActionType::SheriffIdentities::PARAM_LIFETIME_ACTIONS_COUNT] > 0
      action.action_type_params.action_type_params_hash[ActionType::SheriffIdentities::PARAM_LIFETIME_ACTIONS_COUNT] -= 1
